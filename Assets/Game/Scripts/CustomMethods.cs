...
namespace SpaceStuff
{
    ...
    public static class CustomMethods
    {
        ...
        public static float CalculateArrivalTime(float closingAcceleration, float closingSpeed, float distance)
        {
            ...
            if (Mathf.Abs(closingAcceleration) < 0.001f)
            {
                // No acceleration, use simple distance/velocity formula
                if (closingSpeed > 0)
                    arrivalTime = distance / closingSpeed;
            }
            else
            {
                // d = b^2 - 4ac
                float discriminant = closingSpeed * closingSpeed - 2 * closingAcceleration * distance;
                if (discriminant >= 0)
                {
                    float sqrtDiscriminant = Mathf.Sqrt(discriminant);
                    float t1 = (-closingSpeed + sqrtDiscriminant) / closingAcceleration;
                    float t2 = (-closingSpeed - sqrtDiscriminant) / closingAcceleration;
                    arrivalTime = Mathf.Max(t1, t2, 0);
                }
            }
            return arrivalTime;
        }
