...
public class CelestialBody : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            Collider[] collidersInGravity = new Collider[20];
            int hits = Physics.OverlapSphereNonAlloc(transform.position, gravityRadius, collidersInGravity, gravitySettings.affectedLayers);
            for (int i = 0; i < hits; i++)
            {
                Collider colliderInGravity = collidersInGravity[i];
